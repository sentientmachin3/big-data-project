...
\begin{document}
	...
	The project aim was to design a tool which could establish the authorship of a manuscript. The basic workflow of the project (which explained more in detail in the next sections) is the following:\begin{itemize}
    ...
    \subsection{Cloudera Docker and system setup}
		...
		\end{itemize} 
			\item Machine 2: \begin{itemize}
				\item Operating System: \texttt{Linux Manjar}, Kernel version \texttt{5.3.12}, 64bit
...
\begin{table}[h!]
	...
	\begin{tabular}{ccccccc}
		\toprule
		Author   & Articles    & Conjunctions & Prepositions & Avg. Period Length & Dialogues & Commas \\
		\midrule
...
\begin{table}[h!]
	...
	\begin{tabular}{ccccccc}
		...
		\midrule
		\midrule
		Unknown 1 & 0.09512767  & 0.028265182  & 0.10064157   & 15.1724             & 0.19409695  & 0.09657056  \\
		Unknown 2 & 0.10238273  & 0.028543063  & 0.10702407   & 14.196617           & 0.0         & 0.096401095 \\
		Unknown 3 & 0.09730647  & 0.028921144  & 0.1361749    & 20.7383             & 0.0         & 0.09636771  \\
		Unknown 4 & 0.09376974  & 0.021347687  & 0.09346865   & 12.885504           & 0.0         & 0.13776492  \\
		Unknown 5 & 0.09391441  & 0.025601724  & 0.11509038   & 25.835533           & 0.0         & 0.11042987  \\
		\bottomrule
	\end{tabular}
\end{table}
	...
	\begin{figure}[h!]
		...
		\begin{tikzpicture}
			\begin{axis}
			[
			legend entries={Abbott, Shelley, Dickens, Melville},
			legend style={
				at={(1.5,-0.2)},
				anchor=north,
				legend columns=1,
			...
			xticklabels={articles,conjunctions,dialogues,punctuation,prepositions},
			ylabel=Frequencies]
			\addplot table [x=freq, y=abbott, col sep=comma] {frequencies.csv};
			\addplot table [x=freq, y=shelley, col sep=comma] {frequencies.csv};
			\addplot[mark=*, brown] table [x=freq, y=dickens, col sep=comma] {frequencies.csv};
			\addplot[mark=*, black] table [x=freq, y=melville, col sep=comma] {frequencies.csv};
			\end{axis}
		\end{tikzpicture}
	...
	\begin{figure}[h!]
		...
		\begin{tikzpicture}
			...
			xticklabels={articles,conjunctions,dialogues,punctuation,prepositions}]
			\addplot table [data cs=polarrad, x=freq, y=abbott, col sep=comma] {abbott-unknown.csv};
			\addplot table [data cs=polarrad, x=freq, y=unknown1, col sep=comma] {abbott-unknown.csv};
			\addplot table [data cs=polarrad, x=freq, y=unknown2, col sep=comma] {abbott-unknown.csv};
			\addplot table [data cs=polarrad, x=freq, y=unknown3, col sep=comma] {abbott-unknown.csv};
			\end{polaraxis}
		\end{tikzpicture}
	...
	\begin{figure}[h!]
		...
		\begin{tikzpicture}
			...
			yticklabel={\footnotesize\pgfmathprintnumber{\tick}},
			xticklabels={articles,conjunctions,dialogues,punctuation,prepositions}]
			\addplot table [data cs=polarrad, x=freq, y=dickens, col sep=comma] {dickens-unknown.csv};
			\addplot table [data cs=polarrad, x=freq, y=unknown4, col sep=comma] {dickens-unknown.csv};
			\addplot table [data cs=polarrad, x=freq, y=unknown5, col sep=comma] {dickens-unknown.csv};
			\end{polaraxis}
		\end{tikzpicture}
	...
	\begin{figure}[h!]
		...
		\begin{tikzpicture}
			...
			\addplot[ultra thick, black] table [mark=none, x=freq, y=dickens, col sep=comma] {deltas-dickens.csv};
			\addplot[thick,mark=none,blue] table [ x=freq, y=unknown4, col sep=comma] {deltas-dickens.csv};
			\addplot[thick,mark=none,green] table [ x=freq, y=unknown5, col sep=comma] {deltas-dickens.csv};
			\addplot[
                error bars/.cd,
                    y explicit,
					y dir=minus,
				error bar style={dashed,red},
				error mark options={mark=none}
            ] table [
				x=freq,
				y=dickens,
				y error minus expr=\thisrow{dickens}-\thisrow{unknown5},
				col sep=comma
			] {deltas-dickens.csv};
			\addplot[
                error bars/.cd,
				...
				x=freq,
				y=dickens,
				y error minus expr=\thisrow{dickens}-\thisrow{unknown4},
				col sep=comma
            ] {deltas-dickens.csv};
			\end{axis}
		\end{tikzpicture}
		\caption{Deltas Dickens}
	...
	\begin{figure}[h!]
		...
		\begin{tikzpicture}
			...
			\addplot[ultra thick, black] table [mark=none, x=freq, y=abbott, col sep=comma] {deltas-abbott.csv};
			\addplot[mark=none,blue] table [ x=freq, y=unknown1, col sep=comma] {deltas-abbott.csv};
			\addplot[mark=none,red] table [ x=freq, y=unknown2, col sep=comma] {deltas-abbott.csv};
			\addplot[mark=none,green] table [ x=freq, y=unknown3, col sep=comma] {deltas-abbott.csv};
			\addplot[error bars/.cd, y explicit, y dir=minus, error bar style={dashed,red, line width=1pt},
				...
				col sep=comma ] {deltas-abbott.csv};
			\addplot[error bars/.cd, y explicit, y dir=minus, error bar style={dashed,red, line width=1pt},
	...
	\begin{itemize}
		...
		\item \textbf{Input file naming format:} due to difficulties in the parsing process, we decide to format the input file names $author-title.txt$, and the unknown files as $unknown-unknown.txt$.
		
		\item \textbf{Dialogues format:} we noticed that dialogues are not formatted with same character everywhere. We still have to find a solution to this issue.
	\end{itemize}
