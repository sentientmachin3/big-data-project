...
protected void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
	...
	Since the contest has now a lot of lines associated with same value (the \lstinline|IntWritable| instance for ``ones''), the combiner and the reducer are responsible for grouping lines with the same key and count these lines, writing the total into the context. Once the reducing phase is completed, the output is written onto a file into the HDFS (specifically \lstinline|OUTPUT_PATH|).
	...
	More in detail, a complete entry contains the follwing fields: \begin{itemize}
	...
	After this phase, we will have a frequency map for every text of every author, for example: \begin{align*}
		...
		\text{frequencies}&:\begin{cases}
			articles &0.000152 \\
			prepositions &0.000582 \\
			conjunctions &0.00251 \\
			periods &3441 \\
			nwords &134135 \\
